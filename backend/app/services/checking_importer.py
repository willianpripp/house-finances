...
from app.services.pending_supersede import (
    PendingSupersede,
    plan_supersede,
...
class CheckingPreviewActivity(BaseModel):
    ...
    # that replaces a committed pending one updates that ledger row in place,
    # so it is neither an insert nor a duplicate: `will_action` says so, and
    # these carry the same state structurally.
...
class CheckingImportCommitResult(BaseModel):
    ...
    log_notes: list[str]
    # Pending→posted: rows updated in place, and pending rows dropped because
    # their posted twin arrived in the same batch. Neither is an insert.
...
def _will_action_summary(
    ...
    supersede: "PendingSupersede | None" = None,
    superseded_by_posted: bool = False,
) -> str:
    # Pending→posted states come first: they decide what commit does with the
    # row regardless of how it was classified.
    if supersede is not None:
        changed = (
            f", amount {supersede.prior_amount} -> {supersede.amount}"
            if supersede.amount_changed
            else ""
        )
        return (
            f"Replaces pending row #{supersede.prior_id} — update in place, "
            f"no duplicate{changed}"
        )
    if superseded_by_posted:
        return "Skip — superseded by the posted version in this same batch"
...
def build_checking_preview(
    ...
    *,
    ...
    # replace, computed up front so a row's state does not depend on the order
    # the provider listed the two versions in.
    batch_superseded = superseded_pending_ids(
        a.pending_transaction_id for a in parsed.activities
    )
    ...
    for a in parsed.activities:
        ...
        # Ledger convention: a checking debit is a positive charge, so the
        # posted amount is flipped before it is compared with the pending row.
        supersede = plan_supersede(
            session,
            pending_transaction_id=a.pending_transaction_id,
            amount=-a.amount,
        )
        superseded_by_posted = bool(
            a.plaid_transaction_id and a.plaid_transaction_id in batch_superseded
        )
        # A supersede updates the prior row instead of skipping the line, so it
        # is not a duplicate.
        if supersede is not None:
            is_dup_signature = False
            dup_tx_id = None
...
def commit_checking_import(
    ...
    *,
    ...
    # Pending→posted, over the rows this commit will actually process: a posted
    # row the user unticked must not suppress the pending row they kept. See
    # services/pending_supersede.py for the two shapes this handles.
    batch_superseded = superseded_pending_ids(
        a.pending_transaction_id
    ...
    for idx, a in enumerate(parsed.activities):
        if idx in skip_set:
            continue
        # Pending→posted supersede. A pending row whose posted version is in
        # this same batch is the duplicate: committing both is what put two
        # rows in the ledger for one purchase.
        if a.plaid_transaction_id and a.plaid_transaction_id in batch_superseded:
            pending_superseded += 1
            notes.append(
                f"Dropped pending row {a.activity_date} ${abs(a.amount)}: its "
                f"posted version is in this same batch"
            )
            continue
        # A posted row that replaces an already-committed pending one updates
        # that ledger row in place, keeping everything human on it.
        supersede = plan_supersede(
            session,
            pending_transaction_id=a.pending_transaction_id,
            amount=-a.amount,  # checking sign: debit→positive charge
        )
        if supersede is not None:
            note = (
                f"Pending→posted {a.activity_date}: updated ledger row "
